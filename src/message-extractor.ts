...
export class MessageExtractor {
   ...
   * @param messagePack - The container for extracted messages
   * @param startDate - Optional start date for filtering (inclusive)
   */
  async extractMessages(messagePack: MessagePack, startDate?: Date): Promise<void> {
    ...
    messageGroups.forEach((messageGroup) => {
      ...
      // Check date filter
      if (startDate) {
        const messageDate = new Date(parseFloat(tsAttr) * 1000); // Slack timestamp is in seconds
        // Reset time part of messageDate for date-only comparison if needed, 
        // but user requested "1 week" etc which usually implies the beginning of that day.
        // Let's strictly compare vs the calculated start date which should be 00:00:00 of that day.
        if (messageDate < startDate) {
          // Skip messages older than start date
          return;
        }
      }
